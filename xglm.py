...
#from transformers.file_utils import add_code_sample_docstrings, add_start_docstrings, add_start_docstrings_to_model_forward, logging
...
class XGLMAttention(nn.Module):
    ...
    def forward(
        ...
        demos_offset: Optional[list] = None,
        demos_scale: Optional[list] = None,
        re_weight_place: Optional[str] = None,
        ...
        if demos_scale is not None and (re_weight_place=='before_softmax' or re_weight_place=='both'):
            scales = torch.ones_like(attn_weights)
            for i,(beg,end) in zip(range(len(demos_scale)),demos_offset):
                scales[:,:,beg:end] = demos_scale[i]
            attn_weights = attn_weights*scales
        ...
        if demos_scale is not None and (re_weight_place=='after_softmax' or re_weight_place=='both'):
            demos_attn_sum = attn_weights[:,:,demos_offset[0][0]:demos_offset[-1][-1]].sum(dim=-1,keepdim=True)
            demos_attn = torch.zeros(demos_attn_sum.shape[0],demos_attn_sum.shape[1],len(demos_offset),dtype=attn_weights.dtype,device=attn_weights.device)
            for i,(beg,end) in zip(range(len(demos_scale)),demos_offset):
                demos_attn[:,:,i] = demos_scale[i]*attn_weights[:,:,beg:end].sum(dim=-1)
            demos_attn_sum_new = demos_attn.sum(dim=-1,keepdim=True)
            for i,(beg,end) in zip(range(len(demos_scale)),demos_offset):
                attn_weights[:,:,beg:end]=demos_scale[i]*attn_weights[:,:,beg:end]/demos_attn_sum_new*demos_attn_sum
...
class XGLMModel(XGLMPreTrainedModel):
    ...
    def forward(
        ...
        pos_emb_offset: Optional[int] = None,
        demos_offset: Optional[list] = None,
        demos_scale: Optional[list] = None,
        re_weight_place: Optional[str] = None,
        re_weight_layer: Optional[list] = None, 
        ...
        # embed positions
        if pos_emb_offset is not None:
            positions = self.embed_positions(input_ids, inputs_embeds, pos_emb_offset)
        else:
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if re_weight_layer==None or idx in re_weight_layer:
...
class XGLMForCausalLM(XGLMPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            pos_emb_offset=pos_emb_offset,
            demos_offset=demos_offset,
            demos_scale=demos_scale,
            re_weight_place=re_weight_place,
            re_weight_layer=re_weight_layer,
        )
